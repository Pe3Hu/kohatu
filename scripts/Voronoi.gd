...
export var draw_voronoi_cells_convex_hull = true
export var draw_voronoi_edges = false
export var draw_points = true
export var draw_triangle_centers = true
export var debug_mode = false


var default_seed_points = 20
var initial_points
var points
var delaunay
var coordinates = []
...
func get_random_points(seed_points = default_seed_points):
	...
	for i in range(new_points.size()):
		if i >= initial_points.size():
			...
			var x = Global.rng.randi_range(1, int(size.x)-1)
			var y = Global.rng.randi_range(1, int(size.y)-1)
			#print(x,y)
